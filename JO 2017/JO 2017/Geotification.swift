...
class Geotification: NSObject, NSCoding, MKAnnotation {
    ...
    func isEqual(at geotification: Geotification)-> Bool{
        if(self.coordinate.latitude == geotification.coordinate.latitude && self.coordinate.longitude == geotification.coordinate.longitude){
            return true
        }
        else{
            return false
